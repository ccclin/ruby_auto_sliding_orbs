...
class InitData
	#資料物件，含矩陣資料init_data_m、動作物件action（存放class Action_m）
...
class MainRun
	...
	def up_move_check(input_array, last_action)
		#去除上一步動作
		if last_action == "U"
			input_array.action.action_m = input_array.action.action_m - ["D"] 
		end
		if last_action == "D"
			input_array.action.action_m = input_array.action.action_m - ["U"] 
		end
		if last_action == "L"
			input_array.action.action_m = input_array.action.action_m - ["R"] 
		end
		if last_action == "R"
			input_array.action.action_m = input_array.action.action_m - ["L"] 
		end
		if last_action == "UL"
			input_array.action.action_m = input_array.action.action_m - ["DR"] 
		end
		if last_action == "UR"
			input_array.action.action_m = input_array.action.action_m - ["DL"] 
		end
		if last_action == "DL"
			input_array.action.action_m = input_array.action.action_m - ["UR"] 
		end
		if last_action == "DR"
			input_array.action.action_m = input_array.action.action_m - ["UL"]
		end
	    return input_array	
	end
	...
	def rank_add(array_m_init)
		...
		array_m.each_with_index{|x_item, i|
			x_item.each_with_index{|item, j|
	...
	def make_init_array()
		...
		times_index = 0
		am_n = Hash.new
		@init_array.each_with_index{|x_item, i|
			x_item.each_with_index{|item, j|
	...
	def run_case(action_times, combo_times, am_n = @am_n, oblique = "on")
		#主程式method
		am_n_add_one = Hash.new
		times = 1
		while times <= action_times
		#動作迴圈
			times_index = 0
			am_n.each{|array_key, array_value|
			#所有元素進行動作確認（BC確認(bc_check)、上一動作確認(up_move_check)）
				...
				am1.action.action_m.each{|action_item|
				#對該元素可行之動作進行計算
...
in_put_array = [[5,2,3,5,5],[4,3,1,6,2],[3,2,6,2,5],[6,5,4,1,1],[3,2,6,6,4],[4,4,1,5,3]]
# in_put_array = [[6,5,5,2,6],[5,2,2,4,5],[6,3,1,6,6],[4,5,3,6,6],[6,1,5,3,2],[1,2,6,3,3]]
no_rank_times = 1
rank_times = 12
oblique = "on"
